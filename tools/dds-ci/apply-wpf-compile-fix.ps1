...
if (-not $appText.Contains('using System.IO;')) {
    $appText = $appText.Replace('using System.Windows;', "using System.IO;`r`nusing System.Windows;")
}
$needle = '            MessageBox.Show($"Dokkaebi Document Studio could not initialize its native PDF engine.\n\n{ex.Message}", "DDS startup error", MessageBoxButton.OK, MessageBoxImage.Error);'
$replacement = @'
...
'@
if (-not $appText.Contains('release-smoke-error.txt')) {
    if (-not $appText.Contains($needle)) { throw 'DDS startup error line was not found for smoke diagnostics patch' }
    $appText = $appText.Replace($needle, $replacement.TrimEnd("`r","`n"))
}
...
if (-not ([IO.File]::ReadAllText($app).Contains('release-smoke-error.txt'))) { throw 'Smoke diagnostics hook was not applied' }
